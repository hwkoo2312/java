...
<%
	...
	int userNId= Integer.parseInt(request.getParameter("user_nid"));
	...
	String userViewSql="SELECT  USER_NID , NAME , EMAIL , REGI_DT FROM USER WHERE USER_NID = ?  ";
	pstmt =conn.prepareStatement(userViewSql);
	pstmt.setInt(1, userNId);
	rs=pstmt.executeQuery();
	
	String userItemSql="SELECT USER_LIKE_ITEM_NID , LIKE_ITEM_NM , USER_NID FROM USER_LIKE_ITEM WHERE USER_NID=? ";
	pstmt =conn.prepareStatement(userItemSql);
	pstmt.setInt(1, userNId);
	...
	<div class="container">
		...
		<div class="card">
			<div class="card-body">
				...
				<form action="/user/form_ok.jsp" method="post" >
					<div class="form-group">
						<label for="name">Name:</label> <input type="text" id="name" value="<%=rs.getString("NAME") %>" name="user_name" class="form-control">
					</div>
					<div class="form-group">
						<label for="mail">E-mail:</label> <input type="email" id="mail"  value="<%=rs.getString("EMAIL") %>" name="user_email" class="form-control">
					</div>
					<div class="form-group">
					...
					<div  class="form-check">
						...
						<div class=" form-check-inline  ">
							  <input class=" like_item_kind" type="checkbox" id="inlineCheckbox1" name="vehicle1"  value="Bike">
							  <label class="form-check-label" for="inlineCheckbox1"> I have abike</label>
						</div>
						<div class=" form-check-inline ">
							  <input class=" like_item_kind" type="checkbox" id="inlineCheckbox2" name="vehicle2" value="Car">
							  <label class="form-check-label" for="inlineCheckbox2">I have a car</label>
						</div>
					</div>
					<div class="form-group">
						<label for="mail">좋아하는 자전거:</label> 
						<select name="Bikes"  class="form-control">
							<option value="volvo">게리피셔</option>
					...
					<div class="form-group">
						...
						<% while(rsItem.next()){ %>
						<input type="checkbox" name="pet" value="dog" class="form-control" >강아지  <%=rsItem.getString("LIKE_ITEM_NM") %>
				...
				<div class="card-footer text-muted">
					...
					<a href="/user/delete.jsp?user_nid=<%=userNId%>">삭제</a>
				</div>
			</div>
		</div> 
	</div>
